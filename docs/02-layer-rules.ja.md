...
Usecaseは、ModuleやQueryから返された値を業務フローの中で利用できます。<br>
1テーブルのDB検索と保存はModule、複数テーブルの読み取りはQueryに任せ、ORMを直接操作しません。
...
呼び出しと結果による分岐は呼び出し側に残します。

Handlerから直接呼ばれるUsecaseが、必要なトランザクション境界を確定します。<br>
別のUsecaseから呼ばれる共通Usecaseは、呼び出し元のトランザクションに参加し、<br>
自身では`commit`しません。
